...
let savedHikes = document.querySelector('#saved-hikes');

button.addEventListener('click', search);
hikeContainer.addEventListener('click', (event) => {
    const clickedElement = event.target;

    if (!clickedElement.matches('button.addHikeBtn')) {
        return;
    }
    const cardStub = clickedElement.dataset.stub;
    const hikeToSave = hikes.find((hike) => hike.stub === cardStub);

    renderSavedHikes(hikeToSave)
});
...
function hikesTemplate(hike) {
  ...
  <div class="hike-content">
    ...
    <label for="addHikeBtn">Save Hike</label>
    <button class="addHikeBtn" data-stub="${hike.stub}">Add to Saved Hikes</button>
  </div>
</div>`
}

function savedHikesTemplate(hike) {
    return `<div class="hike-card">
  <div class="hike-content">
    <h2>${hike.name}</h2>
    <div class="hike-tags">
...
function renderSavedHikes(hike) {
    let html = savedHikesTemplate(hike);
    savedHikes.innerHTML += html;
}
